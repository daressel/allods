...
--- Выводит в чат многострочное представление таблицы (аналог console.group из JS):
--- первая строка — header, далее ключи/значения с отступом в виде дефисов `-`,
--- `--`, `---` и т.д. Длинные значения обрезаются, циклы ловятся.
---
--- Зачем: чат Allods режет длинные строки, а сериализованный JSON часто длиннее
--- лимита. Построчный вывод — единственный надёжный способ увидеть всё.
---@param header string
---@param value any
---@param limits? ChatTreeLimits
function chatTree(header, value, limits) end
